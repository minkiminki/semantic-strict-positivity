...
(*
  This file shows that our approach still works in "Set" without [impredicative-set].
*)

Inductive subset (A : Set) (P : A -> Prop) : Set :=
| subset_ex : forall x : A, P x -> subset P
.

Definition subset_elmt (A : Set) (P: A -> Prop) (e: subset P) : A :=
  let (a, _) := e in a.
Arguments subset_elmt {A P} e.

Definition subset_proof (A : Set) (P: A -> Prop) (e: subset P) : P (subset_elmt e) :=
  let (a, b) as e0 return (P (subset_elmt e0)) := e in b.
Arguments subset_proof {A P} e.
...
{ Fn :> Set -> Set
; map : forall (X Y: Set) (f: X -> Y), Fn X -> Fn Y
}.

Structure t_prop (Fn: t_data) : Prop := mk_prop
{ map_id: forall (X: Set) x, 
    Fn.(map) (@id X) x = x
; map_comp: forall (X Y Z: Set) (g: Y -> Z) (f: X -> Y) x, 
...
{ Nt :> forall (X: Set) (x: F X), G X

; map_nat: forall (X Y: Set) (f: X -> Y) x,
...
Definition U (Sh: Set) (X: Set) : Set := Sh -> (X + nat).

Definition map Sh (X Y: Set) (f: X -> Y) (u: U Sh X): U Sh Y :=
  fun a => match u a with inl x => inl (f x) | inr b => inr b end.
Arguments map Sh [X Y] f u.

Definition allP Sh (X: Set) (P: X -> Prop) (u: U Sh X) : Prop :=
  forall a x (EQ: u a = inl x), P x.

Lemma map_id Sh : forall (X: Set) x, 
  map Sh (@id X) x = x.
Proof.
...
Lemma map_comp Sh : forall (X Y Z: Set) (g: Y -> Z) (f: X -> Y) x, 
...
Lemma map_injective Sh (X Y: Set) u1 u2 (f: X -> Y)
...
Lemma map_pointwise Sh (X Y: Set) u (f g: X -> Y)
...
Definition back_opt (M: t) (X: Set) (u: SPUF.t _ X) : option (M X) :=
...
Definition back (M: t) (X: Set) (x: X) (u: SPUF.t _ X) : M X :=
  match back_opt M u with None => M.(Functor.map) (fun _ => x) M.(inh) | Some e => e end.

Lemma back_opt_unique (M: t) (X: Set) (fx: M X):
...
Lemma back_unique (M: t) (X: Set) x (fx: M X):
  @back M X x (M.(emb) _ fx) = fx.
Proof. unfold back. rewrite back_opt_unique. auto. Qed.

Definition sig_back (A: Set) (P: A -> Prop) (inh: A -> subset P) (a: A) : subset P :=
  match excluded_middle_informative (P a) with
  | left pf => subset_ex _ a pf
  | _ => inh a
  end.

Lemma sig_on_image (M: t) (A: Set) (P: A -> Prop) (m: M (subset P)):
  ex (unique (SSPF.on_image M (SPUF.map _ subset_elmt (M.(emb) _ m)))).
Proof.
  eexists (M.(Functor.map) subset_elmt m). split.
  - red. rewrite (M.(emb).(NatTrans.map_nat)). eauto.  
  - intros. red in H. apply M.(SSPF.inj).
    rewrite <- H. rewrite M.(emb).(NatTrans.map_nat). auto.
Qed.

Lemma sig_all (M: t) (A: Set) (P: A -> Prop) (m: M (subset P)):
  SPUF.allP P (SPUF.map _ subset_elmt (M.(emb) _ m)).
Proof.
...
Lemma sig_all2 (M: t) (A: Set) (P: A -> Prop) (Q: subset P -> Prop) (m: M (subset P))
    (ALLQ: SPUF.allP (fun a => forall (pf: P a), Q (subset_ex _ _ pf)) 
                     (M.(emb) _ (M.(Functor.map) subset_elmt m))):
...
Lemma sig_back_proj (M: t) (A: Set) (P: A -> Prop) (inh: A -> subset P) (m: SPUF.t M.(Sh) A)
    (ALL: SPUF.allP P m):
  SPUF.map _ subset_elmt (SPUF.map _ (sig_back inh) m) = m.
Proof.
...
Lemma allP_project (M: t) (A: Set) (P: A -> Prop) (m: M A)
    (ALLP: SPUF.allP P (M.(emb) _ m)):
  exists m': M (subset P), m = M.(Functor.map) subset_elmt m'.
Proof.
  destruct (excluded_middle_informative (exists a, P a)) as [EXa|NEXa].
  - destruct EXa as [a Pa]. 
    exists (M.(Functor.map) (sig_back (fun _ => subset_ex P _ Pa)) m).
    apply M.(inj).
    rewrite !M.(emb).(NatTrans.map_nat).
...
(* Example:
   ...
   Inductive Mlist (M: Set -> Set) (X: Set) : Set :=
   | Mnil : Mlist M X
...
Definition Mlist := subset PMlist.

Definition Mnil : Mlist := subset_ex _ Mnil_ PMnil_.

Definition Mcons (hd: M X) (tl: M Mlist) : Mlist :=
  subset_ex _ (Mcons_ hd (SPUF.map _ subset_elmt (M.(SSPF.emb) _ tl))) 
...
Proof.
  ...
  - dependent destruction pf.
    ...
    match goal with [|- context[subset_ex _ _ ?pf]] => generalize pf end.
...
Fixpoint mfix (T: Set) (tnil: T) (tcons: M X -> M Mlist_ -> M T -> T) (l: Mlist_) : T :=
  match l with
  | Mnil_ => tnil
  | Mcons_ hd tl => tcons hd (SSPF.back _ Mnil_ tl) (SSPF.back _ tnil (SPUF.map _ (mfix tnil tcons) tl))
  end.

Lemma mfix_unique (T: Set) (tnil: T) (tcons: M X -> M Mlist_ -> M T -> T) (mfix': Mlist -> T)
    (NIL: mfix' Mnil = tnil)
    (CONS: forall hd tl,
           mfix' (Mcons hd tl) = tcons hd (M.(Functor.map) subset_elmt tl) (M.(Functor.map) mfix' tl)):
  forall l: Mlist, mfix tnil tcons (subset_elmt l) = mfix' l. 
Proof.
...
Lemma len_unique lenM len'
  ...
  forall l: Mlist, len lenM (subset_elmt l) = len' l.
Proof.
  intros. eapply (@Mlist_ind l); eauto.
...
Fixpoint embed (X: Set) (l: list X) (s: list unit) : X + nat :=
  match l with
  | nil => inr 0
  | cons hd tl => 
      match s with 
